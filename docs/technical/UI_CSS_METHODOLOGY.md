...
- `.u-gap-sm`, `.u-gap-md`, `.u-gap-lg` (Token-based)

## 🎯 Phase 3.5: The Utility Belt (Refinement)
A transition towards Atomic CSS to reduce file size and specificity conflicts. All utilities are defined in `Shared/styles/utilities/_utils.scss`.

### Standardized Visibility
Prefer `.u-hidden` (Display) and `.u-fade-in` (Opacity) over legacy classes to ensure compatibility with `SidebarManager`.

---

## 🎯 Phase 4: Design System DNA (Tokens)
Implementation of central control in `Shared/styles/settings/_tokens.scss`.

### Key Systems
1. **Typography**: Single source of truth for `Montserrat` (Primary) and `Roboto` (Secondary).
2. **Glassmorphism**: 3 Levels of transparency (`--surface-glass-1` to `3`).
3. **Elevations**: Standardized shadows and glows (`--elevation-glow`).

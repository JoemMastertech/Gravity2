...
export class OrderLogic {
    ...
    getDrinkModalTitle(productName) {
        if (!productName || !this.currentProduct) {
            return { title: '¿Con qué desea acompañar su bebida?', subtitle: '' };
        }

        const priceType = this.currentProduct.priceType;
        const prefixMap = {
            [CONSTANTS.PRICE_TYPES.BOTTLE]: 'Botella',
            [CONSTANTS.PRICE_TYPES.LITER]: 'Litro',
            [CONSTANTS.PRICE_TYPES.CUP]: 'Copa'
        };
        const prefix = prefixMap[priceType] || 'bebida';

        // Requirement: "¿Cómo desea acompañar su [Prefix] de [ProductName]?"
        const title = `¿Cómo desea acompañar su ${prefix} de ${productName}?`;

        const { message } = this.getDrinkOptionsForProduct(productName);
        let subtitle = message;

        // Requirement: Remove "Puedes elegir: 2 Jarras..." from Litro/Copa
        const isBottle = priceType === CONSTANTS.PRICE_TYPES.BOTTLE;
        ...
        if (isBottle) {
            ...
            // No bottle-specific descriptions for Litro/Copa
            subtitle = '';
        }

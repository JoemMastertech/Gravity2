...
const ProductRenderer = {
  ...
  _createTableElement: function (tableClass, categoryTitle) {
    ...
    table.className = `${tableClass} u-full-width`;
  ...
  createProductGrid: function (container, data, fields, categoryTitle) {
    // 1. Create Wrapper Container (for Background Formatting)
    const wrapper = document.createElement('div');
    wrapper.className = 'grid-view-container u-p-xl u-flex-column u-flex-align-c';
    ...
    wrapper.appendChild(titleElement); // Title first
    ...
    wrapper.appendChild(grid); // Grid second
    container.appendChild(wrapper); // Add wrapper to main container
  ...
  /**
   * MIGRATED: Unified Media Modal via ModalSystem
   * Handles both Video and Image viewing.
   */
  showMediaModal: function (config) {
    const { type, url, title, category } = config;

    const contentWrapper = document.createElement('div');
    contentWrapper.className = `media-modal-wrapper ${type}-modal`;
    if (category) contentWrapper.setAttribute('data-category', category);
    ...
    if (type === 'video') {
      ...
      video.className = 'media-content video-content u-full-width';
      video.style.borderRadius = 'var(--radius-md, 8px)';

      video.onerror = () => {
        Logger.error(`Media Modal: Video failed to load: ${url}`);
        contentWrapper.innerHTML = `<div class="error-message u-p-lg text-center">Video no disponible</div>`;
      };
      ...
      img.src = url;
      img.alt = title;
      img.className = 'media-content image-content u-full-width';
      img.style.borderRadius = 'var(--radius-md, 8px)';

      img.onerror = () => {
        Logger.error(`Media Modal: Image failed to load: ${url}`);
        contentWrapper.innerHTML = `<div class="error-message u-p-lg text-center">Imagen no disponible</div>`;
      };

      contentWrapper.appendChild(img);
    }

    const modalSize = type === 'video' ? 'large sys-modal-top' : 'media-image sys-modal-top';
    ...
    const licoresHTML = `
      <div class="grid-view-container u-p-xl">
        ...
        <div class="subcategory-prompt">
          <h3 class="page-title" data-translate="category.prompt.choose" data-original-text="Elige una categoría" data-namespace="categories">Elige una categoría</h3>
        </div>
      </div>
    `;

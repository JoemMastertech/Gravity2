...
initDebugPanel() {
  ...
  if (debugPanel) {
    ...
    controls.style.borderBottom = '1px solid #666';
    ...
    clearBtn.style.backgroundColor = '#f57c00'; // Orange
    ...
    deleteDbBtn.style.backgroundColor = '#d32f2f'; // Red
    deleteDbBtn.style.color = 'white';
    deleteDbBtn.style.border = 'none';
    deleteDbBtn.style.padding = '4px 8px';
    deleteDbBtn.style.borderRadius = '3px';
    deleteDbBtn.style.cursor = 'pointer';
    ...
    // Insert controls at the top of the panel
    debugPanel.insertBefore(controls, debugPanel.firstChild);
  }
}
...
logToPanel(message, type = 'info') {
  ...
  if (debugPanel) {
    ...
    debugPanel.innerHTML += `<div style="color:${color}; margin-bottom:2px; font-size:11px;">[${new Date().toLocaleTimeString()}] ${message}</div>`;
...
isValidTranslation(sourceText, translatedText, targetLanguage) {
  ...
  try {
    ...
    console.log(`[TranslationService] Requesting translation for: "${sourceText}" to ${targetLanguage}`);
    ...
    if (newTranslation) {
      console.log(`[TranslationService] Received translation: "${newTranslation}"`);
  ...
  try {
    console.log(`[TranslationService] Saving to Supabase: ${textKey}`);
    const { error } = await this.supabase
      .from('translations')
      .upsert({
        text_key: textKey,
    ...
    if (error) {
      console.error('[TranslationService] Error saving to Supabase:', error);
      Logger.error('Error saving translation to Supabase:', error);
    } else {
      console.log(`[TranslationService] Saved to Supabase successfully: ${textKey}`);
      Logger.debug(`Translation saved to Supabase: ${textKey} -> ${targetLanguage}`);
    }
  } catch (error) {
    console.error('[TranslationService] Exception saving to Supabase:', error);
    Logger.error('Error saving translation to Supabase:', error);
  }
}

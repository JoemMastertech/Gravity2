...
class SettingsManager {
  ...
  async toggleSettingsMenu() {
    try {
      const module = await import('../../Interfaces/web/ui-adapters/components/SettingsDrawer.js');
      const settingsDrawer = module.default;
      settingsDrawer.toggle();
    } catch (err) {
      console.error('SettingsManager: Failed to load SettingsDrawer', err);
    }
  }
  ...
  async showPanel(panel) {
    ...
    try {
      const module = await import('../../Interfaces/web/ui-adapters/components/SettingsDrawer.js');
      module.default.showPanel(panel.id);
    } catch (err) {
      // Fallback
      const allPanels = document.querySelectorAll('.settings-panel');
      allPanels.forEach(p => p.classList.add('u-hidden'));
      panel.classList.remove('u-hidden');
      panel.classList.add('u-fade-in');
    }
  }
  ...
  async applySavedPreferences() {
    try {
      ...
      if (savedTheme) {
        // High-level attribute should already be set by Early Injector in index.html
        // but we ensure it here just in case.
        document.body.setAttribute('data-theme', savedTheme);

        // Highlight the button and update logos
        ...
        // Only trigger changeTheme (which reloads video) if something went wrong 
        // with the early injector or if we need to sync state.
        if (!window.__initialThemeApplied) {
        ...
        this.updateLogosForTheme(defaultTheme);
      }
    } catch (error) {
